...
# 电机ID从1开始连续编号，数量由第二个命令行参数设置。
MOTOR_IDS=()
for ((motor_id=1; motor_id<=MOTOR_COUNT; motor_id++)); do
    MOTOR_IDS+=("$motor_id")
done
...
disable_all_motors() {
    ...
    for IF in "${INTERFACES[@]}"; do
        for m in "${MOTOR_IDS[@]}"; do
            MID=$(printf '%04X' "$m")
            cansend "$IF" "7FF#${MID}0007" >/dev/null 2>&1 || true
        done
    done
}

cleanup() {
    echo -e "\n${COL_YEL}[!] Stopping tests and disabling configured motors...${COL_RST}"
    stop_error_monitor
    disable_all_motors
